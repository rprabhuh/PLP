...
public class Scanner {
	...
	public void scan() {
		...
		for(int i=0;i<=stream.inputChars.length;i++) {
			...
			case START:
				if(i==stream.inputChars.length) {
					...
					if(k == ILLEGAL_CHAR){
						...
						if(lookup(i+1)=='.') {
							Token t = stream.new Token(RANGE, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
					...
					if(k==ASSIGN) {
						if(lookup(i+1)=='='){
							Token t = stream.new Token(EQUAL, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
						...
						if(lookup(i+1)=='='){
							Token t = stream.new Token(NOTEQUAL, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
						...
						if(lookup(i+1)=='='){
							...
							Token t = stream.new Token(LSHIFT, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
						...
						if(lookup(i+1)=='='){
							...
							Token t = stream.new Token(RSHIFT, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
						...
						if(lookup(i+1)=='>'){
							Token t = stream.new Token(ARROW, i, i+2, linecount);
							stream.tokens.add(t);
							i++;
							beg=beg+1;
							continue;
			...
			case IDENT:
				if(i==stream.inputChars.length) {
					Token t1 = stream.new Token(getKeywordLit(String.valueOf(stream.inputChars, beg, i - beg)), beg, i, linecount);
					Token t2 = stream.new Token(EOF,i,i,linecount);
					stream.tokens.add(t1);
					stream.tokens.add(t2);
