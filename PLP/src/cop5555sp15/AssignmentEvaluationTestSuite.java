...
public class AssignmentEvaluationTestSuite {
	...
	public void separators1() {
		...
		String[] expectedTexts = { ",", "(", ")", "[", "]", "" }; // need empty
																	// string
																	// for eof
		assertArrayEquals(expectedKinds, makeKindArray(stream));
		assertArrayEquals(expectedTexts, makeTokenTextArray(stream));
	...
	public void operators2() {
		...
		String[] expectedTexts = { ">=", "<<", ">>", "" }; // need empty string
															// for eof
		assertArrayEquals(expectedKinds, makeKindArray(stream));
		assertArrayEquals(expectedTexts, makeTokenTextArray(stream));
	}
	...
	public void twoCharOperators4() {
		String input = "<<<<=!=!=";
		TokenStream stream = scanInput(input);
		Kind[] expectedKinds = { LSHIFT, LSHIFT, ASSIGN, NOTEQUAL, NOTEQUAL,
				EOF };
		String[] expectedTexts = { "<<", "<<", "=", "!=", "!=", "" }; // need
																		// empty
																		// string
																		// for
																		// eof
	...
	public void illegalChars() {
		String input = "# ^ ~ ` '";
		TokenStream stream = scanInput(input);
		Kind[] expectedKinds = { ILLEGAL_CHAR, ILLEGAL_CHAR, ILLEGAL_CHAR,
				ILLEGAL_CHAR, ILLEGAL_CHAR, EOF };
		String[] expectedTexts = { "#", "^", "~", "`", "'", "" }; // need empty
																	// string
																	// for eof
		assertArrayEquals(expectedKinds, makeKindArray(stream));
	...
	public void identsTest1() {
		...
		TokenStream stream = scanInput(input);
		Kind[] expectedKinds = { IDENT, IDENT, IDENT, IDENT, IDENT, EOF };
		String[] expectedTexts = { "$", "$8", "a$a", "$$$", "aa$9", "" }; // need
																			// empty
																			// string
																			// for
																			// eof
	...
	public void identsTest2() {
		...
		TokenStream stream = scanInput(input);
		Kind[] expectedKinds = { IDENT, IDENT, IDENT, IDENT, IDENT, EOF };
		String[] expectedTexts = { "_", "_8", "a_a", "___", "aa_9", "" }; // need
																			// empty
																			// string
																			// for
																			// eof
	...
	public void intLiteralTest1() {
		...
		Kind[] expectedKinds = { INT_LIT, INT_LIT, INT_LIT, INT_LIT, INT_LIT,
				INT_LIT, EOF };
		String[] expectedTexts = { "5", "0", "0", "1", "100", "1251", "" }; // need
																			// empty
																			// string
																			// for
																			// eof
